...
# Batch 8 bài quá nhỏ khiến 110 bài phải chia 14 batch → quá nhiều lần gọi API,
# đụng rate limit hàng loạt cả 3 provider. Tăng lại lên 25 để giảm còn ~5 batch.
...
def process_batch_with_ai(batch_articles, batch_label=""):
    ...
    for provider in AI_PROVIDERS:
        ...
        for model_idx, model in enumerate(provider["models"]):
            # Giảm số lần retry để tiết kiệm quota — mỗi retry tốn thêm 1 request
            # tính vào rate limit, càng retry nhiều càng dễ cháy quota nhanh hơn.
            max_retries = 1 if pname == "gemini" else 2
                ...
                try:
                    ...
                    if status == 429:
                        if pname == "gemini":
                            ...
                            # Groq/OpenRouter: retry với backoff (giảm còn tối đa 1 lần chờ)
                            if retry_attempt < max_retries - 1:
                                ...
                                print(f"  ⚠️  [{pname}] {model} đã retry vẫn 429. Chuyển model khác...")
                                break
...
def process_with_ai(articles):
    ...
    for i, batch in enumerate(batches, 1):
        ...
        # Nghỉ giữa các batch để không dồn dập gọi API liên tiếp,
        # giúp các provider có thời gian "hồi" quota theo phút.
        if i < len(batches):
            time.sleep(6)
